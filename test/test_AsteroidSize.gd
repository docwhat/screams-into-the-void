...
func test_all_have_required_values():
  for size : AsteroidSize in AsteroidSize.sizes:
    assert_true(size.name.length() >= 3, "Name should be longer than 3 characters for %s" % size.resource_path)
    assert_true(size.shape_radius > 1.0, "Shape radius should be greater than 0 for %s" % size.name)
    assert_true(size.shape_max_radius_delta > 0.0, "Max radius delta should be non-negative for %s" % size.name)
    assert_true(size.shape_number_of_points >= 3, "Number of points should be at least 3 for %s" % size.name)
...
func test_generatePolygon_first_point_at_radius():
  ...
  assert_almost_eq(first_point.x, asteroid_size.shape_radius, 0.001, "First point x should be at radius")
