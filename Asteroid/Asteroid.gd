...
func rebuild() -> void:
  ...
  assert(radius > 0.0)
  var m : float = matter_collection.matter_mass() + 1.0001
  assert(m > 1.0)
  ...
  print_rich("%s %s \n  m-r:    %f \n  m-m:    %f \n  i-r:    %f \n  i-m:    %f" % [
    asteroid_size.name, asteroid_kind.name,
    1_000.0 * radius, get_mass(),
    1_000_000.0 * radius, get_inertia()])
...
func launch(screen_size : Vector2, player_coord : Vector2) -> Node:
  ...
  print_rich("Launch: %s %s  ->  velocity: %s  torque: %f" % [
    asteroid_kind.name, asteroid_size.name,
    velocity, torque
  ])
...
func _random_matter() -> MatterCollection:
  var collection = MatterCollection.new()
  ...
  for m : MatterCollection.Matter in matter:
    ...
    collection.set_amount(m, amount)
    if Global.debug_asteroid_kind:
